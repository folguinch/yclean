...
from goco_helpers import argparse_process as args_proc
from goco_helpers import utils
import numpy as np
...
def _check_env(args: NameSpace) -> None:
    ...
    # Main directory
    ycleandir = Path(args.basedir)
    ycleandir = ycleandir / 'yclean'

    # Check dir
    if not args.resume or not ycleandir.is_dir():
        if ycleandir.is_dir():
            args.log.warn('Emptying YCLEAN directory: %s', ycleandir)
            for aux in ycleandir.glob('*'):
                if aux.is_dir():
                    os.system(f'rm -rf {aux}')
        else:
            ycleandir.mkdir(parents=True, exist_ok=True)
...
def run_yclean(args: Optional[List] = None) -> None:
    ...
    logfile = f'debug_yclean_{logfile}.log'
    args_parents = [parents.logger(logfile)]
    parser = argparse.ArgumentParser(
        description='Execute YCLEAN from command line inputs',
        add_help=True,
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        parents=args_parents,
        conflict_handler='resolve',
    )
    parser.add_argument('-b', '--basedir', default='', type=str,
                        help='Base directory')
    ...
    args.log.info('Configuration file: %s', args.configfile)
    args.log.info('MS data: %s', args.uvdata)
