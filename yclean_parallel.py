...
casalog.post("Secondary Lobe PSF Level: %f" % secondary_lobe_level)
...
while limitLevelSNR>1.5:
    ...
    casalog.post("Iter "+str(it)+": SNR of Maximum Residual:-----"+str(limitLevelSNR/secondary_lobe_level))
    # threshold needs to be (slightly?) above limitLevelSNR 
    threshold=str(limitLevelSNR*2*rms*1.e3)+'mJy' 
    casalog.post("Iter "+str(it)+": SNR of threshold:-----"+str(limitLevelSNR))
    ...
    casalog.post("Iter "+str(it)+": SNR of masklevel: -----"+str(masklevel/rms))
    ...
    if xstatnew['max']>xstat['max']:
        ...
        xstat=xstatnew
    limitLevelSNR=float(xstat['max']/rms*secondary_lobe_level)
    fitsimage = imagename+'.tc_'+str(it)+'.fits'
